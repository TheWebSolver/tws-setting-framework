...
final class Without_Fields extends Setting_Component {
    ...
    public function __construct() {
        parent::__construct(
            [
                __CLASS__ => [
                    'menu_slug'     => HZFEX_SETTING_MENU, // uses default if not set
                    'page_title'    => __( 'Welcome', 'tws-core' ),
                    'menu_title'    => __( 'Welcome Menu', 'tws-core' ),
                    'cap'           => 'manage_options', // wordpress user capability to view page and edit/update page section fields
                    'slug'          =>'tws_without_fields_page', // submenu page slug
                    'icon'          => HZFEX_SETTING_URL . 'assets/graphics/files-icon.svg', // icon that displays on before page navigation title (files icon before page title Welcome)
                ]
            ]
        );
    }
